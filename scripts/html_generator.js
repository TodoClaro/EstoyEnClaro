...
function generateHtmlPage(c) {
  ...
  const quickActionsHtml = `
        ...
        <div class="p-3 sm:p-4 bg-slate-900 text-white grid ${hasRealAddress ? 'grid-cols-3' : 'grid-cols-2'} gap-2 sm:gap-3">
          <a href="${waUrl}" target="_blank" rel="noopener noreferrer" class="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2.5 sm:py-3 px-2 sm:px-3 rounded-xl sm:rounded-2xl text-[11px] sm:text-sm transition flex items-center justify-center gap-1.5 sm:gap-2 shadow-md whitespace-nowrap text-center">
            <i data-lucide="message-circle" class="w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0"></i>
            <span>WhatsApp</span>
          </a>
          <a href="${telUrl}" class="w-full bg-slate-800 hover:bg-slate-700 text-white font-bold py-2.5 sm:py-3 px-2 sm:px-3 rounded-xl sm:rounded-2xl text-[11px] sm:text-sm transition flex items-center justify-center gap-1.5 sm:gap-2 border border-slate-700 whitespace-nowrap text-center" title="Llamar">
            <i data-lucide="phone" class="w-3.5 h-3.5 sm:w-4 sm:h-4 text-emerald-400 shrink-0"></i>
            <span>Llamar</span>
          </a>
          ${hasRealAddress ? `
          <a href="${mapUrl}" target="_blank" rel="noopener noreferrer" class="w-full bg-sky-600 hover:bg-sky-700 text-white font-bold py-2.5 sm:py-3 px-2 sm:px-3 rounded-xl sm:rounded-2xl text-[11px] sm:text-sm transition flex items-center justify-center gap-1.5 sm:gap-2 shadow-xs whitespace-nowrap text-center">
        ...
        <div class="relative h-64 sm:h-80 bg-slate-900">
          ...
          <div class="absolute bottom-4 left-4 right-4 text-white space-y-1">
            ...
            <div class="flex items-center gap-1.5 text-xs text-slate-200 pt-0.5">
              <i data-lucide="${hasRealAddress ? 'map-pin' : 'truck'}" class="w-3.5 h-3.5 text-cyan-400 shrink-0"></i>
              <span>${escapeHtml(hasRealAddress ? direccionCompleta : `${c.zona || 'Claromecó'} (A domicilio / Consultar)`)}</span>
            </div>
          </div>
        </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-3.5">
            <div class="bg-slate-50 p-4 sm:p-5 rounded-2xl border border-slate-200/80 flex items-start gap-3.5">
              ...
              <div class="space-y-0.5">
                <span class="text-slate-400 font-bold uppercase tracking-wider text-[10px] sm:text-xs">Horarios de Atención</span>
            ...
            <div class="bg-slate-50 p-4 sm:p-5 rounded-2xl border border-slate-200/80 flex items-start gap-3.5">
              ...
              <div class="space-y-0.5">
                <span class="text-slate-400 font-bold uppercase tracking-wider text-[10px] sm:text-xs">${hasRealAddress ? 'Dirección' : 'Modalidad de Atención'}</span>
