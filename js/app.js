...
// --------------------------------------------------------------------
// 1. Modal de Búsqueda en tiempo real (Estilo Mercado Libre en Móvil / Flotante en PC)
// --------------------------------------------------------------------
function initSearchModal() {
  const searchModal = document.getElementById('search-modal');
  const openButtons = document.querySelectorAll('[data-open-search]');
  const closeButtons = document.querySelectorAll('#close-search-btn, #close-search-btn-mobile');
  ...
  // Cerrar al clickear fuera del contenedor en desktop
  ...
  function renderSearchResults(q) {
    ...
    if (!q) {
      searchResults.innerHTML = `
        <div class="py-3 sm:py-5 space-y-4">
          <div>
            <div class="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2.5 flex items-center gap-1.5">
              <i data-lucide="sparkles" class="w-3.5 h-3.5 text-amber-500"></i>
              <span>Búsquedas populares en Claromecó</span>
            </div>
            <div class="flex flex-wrap items-center gap-2">
              <button onclick="setSearchTerm('Parrilla')" class="bg-slate-100 hover:bg-sky-50 active:bg-sky-100 text-slate-800 text-xs font-semibold px-3 py-2 rounded-full transition flex items-center gap-1.5 cursor-pointer border border-slate-200/60">
                <span>🥩</span> Parrillas & Asadores
              </button>
              <button onclick="setSearchTerm('Cabañas')" class="bg-slate-100 hover:bg-sky-50 active:bg-sky-100 text-slate-800 text-xs font-semibold px-3 py-2 rounded-full transition flex items-center gap-1.5 cursor-pointer border border-slate-200/60">
                <span>🏡</span> Cabañas Dunamar
              </button>
              <button onclick="setSearchTerm('Rabas')" class="bg-slate-100 hover:bg-sky-50 active:bg-sky-100 text-slate-800 text-xs font-semibold px-3 py-2 rounded-full transition flex items-center gap-1.5 cursor-pointer border border-slate-200/60">
                <span>🦑</span> Rabas & Pescados
              </button>
              <button onclick="setSearchTerm('Farmacia')" class="bg-slate-100 hover:bg-sky-50 active:bg-sky-100 text-slate-800 text-xs font-semibold px-3 py-2 rounded-full transition flex items-center gap-1.5 cursor-pointer border border-slate-200/60">
                <span>💊</span> Farmacias de Guardia
              </button>
              <button onclick="setSearchTerm('Kayak')" class="bg-slate-100 hover:bg-sky-50 active:bg-sky-100 text-slate-800 text-xs font-semibold px-3 py-2 rounded-full transition flex items-center gap-1.5 cursor-pointer border border-slate-200/60">
                <span>🚣</span> Kayaks & Arroyo
              </button>
              <button onclick="setSearchTerm('Kiosco 24')" class="bg-slate-100 hover:bg-sky-50 active:bg-sky-100 text-slate-800 text-xs font-semibold px-3 py-2 rounded-full transition flex items-center gap-1.5 cursor-pointer border border-slate-200/60">
                <span>🏪</span> Kioscos 24 Horas
              </button>
              <button onclick="setSearchTerm('Mecánica')" class="bg-slate-100 hover:bg-sky-50 active:bg-sky-100 text-slate-800 text-xs font-semibold px-3 py-2 rounded-full transition flex items-center gap-1.5 cursor-pointer border border-slate-200/60">
                <span>🛞</span> Auxilio 4x4 & Mecánica
              </button>
            </div>
          </div>
          ...
          <div class="pt-2 border-t border-slate-100">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
              <a href="/categorias/gastronomia.html" class="p-2.5 rounded-xl bg-slate-50 hover:bg-sky-50 border border-slate-200/80 flex items-center gap-2 transition">
                <span class="text-sm">🍽️</span>
                <span class="text-xs font-bold text-slate-800">Gastronomía</span>
              </a>
              <a href="/categorias/alojamiento.html" class="p-2.5 rounded-xl bg-slate-50 hover:bg-sky-50 border border-slate-200/80 flex items-center gap-2 transition">
                <span class="text-sm">🛏️</span>
                <span class="text-xs font-bold text-slate-800">Alojamiento</span>
              </a>
              <a href="/categorias/almacenes_kioscos.html" class="p-2.5 rounded-xl bg-slate-50 hover:bg-sky-50 border border-slate-200/80 flex items-center gap-2 transition">
                <span class="text-sm">🛒</span>
                <span class="text-xs font-bold text-slate-800">Almacenes</span>
              </a>
              <a href="/categorias/servicios_oficios.html" class="p-2.5 rounded-xl bg-slate-50 hover:bg-sky-50 border border-slate-200/80 flex items-center gap-2 transition">
                <span class="text-sm">🔧</span>
                <span class="text-xs font-bold text-slate-800">Servicios</span>
              </a>
            </div>
          </div>
        </div>
    ...
    if (matches.length === 0) {
      searchResults.innerHTML = `
        <div class="py-10 text-center space-y-2">
          ...
          <div class="text-xs sm:text-sm font-bold text-slate-800">No encontramos resultados para "${q}"</div>
          <p class="text-xs text-slate-500 max-w-xs mx-auto">
            Probá buscar por categoría (ej. <em>Parrillas</em>, <em>Cabañas</em>, <em>Farmacias</em>) o por zona (<em>Dunamar</em>, <em>Claromecó</em>).
          </p>
        </div>
      `;
      if (window.lucide) window.lucide.createIcons();
    ...
    searchResults.innerHTML = `
      <div class="space-y-2.5">
        <div class="flex items-center justify-between text-[11px] font-bold text-slate-400 uppercase tracking-wider">
          <span>Comercios encontrados (${matches.length})</span>
          <span class="text-sky-600 font-semibold normal-case">Toca para ver ficha</span>
        </div>
        ${matches.map(c => `
          <a href="/comercios/${c.slug}.html" class="flex items-center justify-between p-3 rounded-2xl bg-slate-50/70 hover:bg-sky-50/80 active:bg-sky-100 border border-slate-200 transition group">
            <div class="flex items-center gap-3 min-w-0">
              ...
              <div class="min-w-0">
                <div class="flex items-center gap-1.5 flex-wrap">
                  <h4 class="text-xs sm:text-sm font-bold text-slate-900 group-hover:text-sky-700 truncate">${c.nombre}</h4>
                  ${c.plan?.destacado_categoria ? '<span class="bg-amber-100 text-amber-800 text-[10px] font-extrabold px-1.5 py-0.2 rounded shrink-0">Destacado</span>' : ''}
                </div>
            ...
            <i data-lucide="chevron-right" class="w-4 h-4 text-slate-400 group-hover:translate-x-0.5 transition shrink-0 ml-2"></i>
          </a>
        `).join('')}
      </div>
    `;
